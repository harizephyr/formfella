    ...
    for i, field in enumerate(empty_fields, 1):
        ...
        if field['context_lines']:
            ...
            for j, line in enumerate(field['context_lines']):
                words = line.split()
                # Get the last 2 words if they exist, or fewer if the line is short
                prev_words = ' '.join(words[-2:]) if len(words) > 1 else line
    ...
    if unselected_checkboxes:
        ...
        for i, element in enumerate(unselected_checkboxes, 1):
            ...
            if should_select == 'quit':
                ...
                print("⏭️  Checkbox will remain unselected")
...
# Your existing imports and Textract logic remain above...
openai.api_key = os.getenv("OPENAI_API_KEY")

def llm_interactive_form_filling(fillable_fields, selection_elements):
    """
    One-by-one interactive form filling using OpenAI to phrase each question.
    Fills both text fields and checkboxes in sequence.
    """
    filled_data = []
    ...
    # Process text fields one at a time
    for idx, field in enumerate(empty_fields, 1):
        label = field.get('field_label', 'Unknown Field')
        context = " | ".join(field.get('context_lines', []))

        # Ask LLM to phrase a natural question for this field
        prompt = f"""
        You are a form-filling assistant.
        The field label is: '{label}'
        Context from the document: {context}
        Generate a short, clear question to ask the user to provide the value.
        ...
        try:
            llm_resp = openai.ChatCompletion.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a helpful assistant for filling PDF forms."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0
            )
            question = llm_resp.choices[0].message['content'].strip()
        except Exception as e:
            print(f"⚠️ LLM error: {e}")
            question = f"Enter value for '{label}':"

        print(f"\n--- Field {idx}/{len(empty_fields)} (Page {field['page']}) ---")
        print(f"📄 Context: {context}")
        user_val = input(f"{question} ").strip()
    ...
    # Process checkboxes one at a time
    unselected_checkboxes = [elem for elem in selection_elements if elem['status'] == 'NOT_SELECTED']
    for idx, checkbox in enumerate(unselected_checkboxes, 1):
        coords = checkbox['coordinates']
        prompt = f"""
        You are a form assistant.
        This is a checkbox at coordinates {coords} on page {checkbox['page']}.
        Ask the user in a simple yes/no form if they want to select this checkbox.
        Example: "Do you want to check the 'Accept Terms' box?"
        """
        ...
        try:
            ...
            print(f"⚠️ LLM error: {e}")
            question = f"Select checkbox at {coords}? (y/n):"
        ...
        if ans in ['y', 'yes']:
            ...
            print("⏭️ Checkbox left unselected")
...
if __name__ == "__main__":
    ...
    user_data = llm_interactive_form_filling(fillable_fields, selection_elements)
    
    print(f"\n🔍 MAIN DEBUG: user_data length = {len(user_data)}")
    print(f"🔍 MAIN DEBUG: user_data = {user_data}")
    ...
    if user_data and len(user_data) > 0:
        ...
        for item in user_data:
            if item['type'] == 'text_field':
                print(f"  📝 '{item.get('field_label', 'Unknown')}' = '{item['user_value']}'")
            elif item['type'] == 'checkbox':
                print(f"  ☑️  Checkbox at page {item['page']} = Selected")
    else:
        print(f"\n🤷 No data was entered. (user_data length: {len(user_data) if user_data else 'None'})")
